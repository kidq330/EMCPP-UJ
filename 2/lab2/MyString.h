...
class MyString : public std::string {
 ...
 public:
  static bool logging;
  MyString() : Base() { logger("Default constructor"); }
  MyString(const char* c) : Base(c) { logger("Constructor from C string"); }
  MyString(const MyString& s) : Base(s) { logger("Copy constructor"); }
  MyString(MyString&& s) : Base(std::move(s)) { logger("Move constructor"); }
